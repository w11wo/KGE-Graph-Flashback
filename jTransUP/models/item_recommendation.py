...
def train_loop(
    FLAGS, model, trainer, train_dataset, eval_datasets, user_total, item_total, logger, vis=None, is_report=False
):
    ...
    for _ in range(trainer.step, FLAGS.training_steps):
        ...
        if (
            FLAGS.early_stopping_steps_to_wait > 0
            and (trainer.step - trainer.best_step) > FLAGS.early_stopping_steps_to_wait
        ):
            logger.info(
                "No improvement after " + str(FLAGS.early_stopping_steps_to_wait) + " steps. Stopping training."
            )
            if pbar is not None:
                pbar.close()
            break
        if trainer.step % FLAGS.eval_interval_steps == 0:
            ...
            if trainer.step > 0 and len(performances) > 0:
                ...
                # visuliazation
                if vis is not None:
                    vis.plot_many_stack({"Rec Train Loss": total_loss}, win_name="Loss Curve")
                    f1_vis_dict = {}
                    p_vis_dict = {}
                    r_vis_dict = {}
                    hit_vis_dict = {}
        ...
        if FLAGS.model_type in ["transup", "transupb"]:
            ...
            losses += (
                orthogonalLoss(model.pref_embeddings.weight, model.pref_norm_embeddings.weight)
                + normLoss(user_embeddings)
                + normLoss(item_embeddings)
                + normLoss(model.pref_embeddings.weight)
            )
...
def run(only_forward=False):
    ...
    train_dataset, eval_datasets, u_map, i_map = load_data(
        dataset_path, eval_files, FLAGS.batch_size, logger=logger, negtive_samples=FLAGS.negtive_samples
    )
