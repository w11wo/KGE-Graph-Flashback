    ...
    with open(filename, "r", encoding="utf-8") as fin:
        for line in fin:
            ...
            if item_vocab is not None and kg_vocab is not None:
                if i_id not in item_vocab or kg_uri not in kg_vocab:
                    continue
...
def parseKGResults(log_filename):
    ...
    print(
        "parse {} head predictions, avgerage {} correct and {} wrong!".format(
            len(head_results), head_correct_num / len(head_results), head_wrong_num / len(head_results)
        )
    )
    print(
        "parse {} tail predictions, avgerage {} correct and {} wrong!".format(
            len(tail_results), tail_correct_num / len(tail_results), tail_wrong_num / len(tail_results)
        )
    )
        ...
        for line in fin:
            ...
            for i_id in gold_ids:
                ...
                if m is not None:
                    ...
                    total_num += 1
    print(
        "parse {} item/rel pairs, avgerage {} users, invovled {} relations!".format(
            len(rating_dict), total_num / len(rating_dict), len(rel_set)
        )
    )
    ...
    with open(output_file, "w", encoding="utf-8") as fout:
        for tr in head_results1:
            ...
            outputHead(
                ...
                e_map_reverse=e_map_reverse,
                r_map_reverse=r_map_reverse,
            )

        for hr in tail_results1:
            if hr not in tail_results2:
                continue
            correct1, wrong1 = tail_results1[hr]
            correct2, wrong2 = tail_results2[hr]

            target_tails = correct2 - correct1
            if len(target_tails) == 0 or hr not in triple_tail_dict:
                continue
...
def outputHead(
    ...
    out_reverse = False
    if (
        u_map_reverse is not None
        and i_map_reverse is not None
        and e_map_reverse is not None
        and r_map_reverse is not None
    ):
        out_reverse = True
    t = tr[0]
    r = tr[1]
    if out_reverse:
        out_str = "tail:{}\trel:{}\ttarget_head:[{}]".format(
            e_map_reverse[t], r_map_reverse[r], ",".join([e_map_reverse[h] for h in target_heads])
        )
    else:
        out_str = "tail:{}\trel:{}\ttarget_head:[{}]".format(
            e_map_reverse[t], r_map_reverse[r], ",".join([str(h) for h in target_heads])
        )
    ...
    for h in remap_hs:
        ...
        for gh in remap_ghs:
            ...
            if len(target_users) > 0:
                if out_reverse:
                    tmp_str = "{},{},[{}]".format(
                        e_map_reverse[i2kg_map[h]],
                        e_map_reverse[i2kg_map[gh]],
                        ",".join([str(u_map_reverse[u]) for u in target_users]),
                    )
                else:
                    ...
                    if len(target_users) > 0:
                        if out_reverse:
                            tmp_str = "{},{},{},[{}]".format(
                                ...
                                e_map_reverse[i2kg_map[gh]],
                                r_map_reverse[tmp_r],
                                ",".join([u_map_reverse[u] for u in target_users]),
                            )
                        else:
                            tmp_str = "{},{},{},[{}]".format(
                                h, gh, r_map_reverse[tmp_r], ",".join([str(u) for u in target_users])
                            )
                        out_str += "\tusers:{}".format(tmp_str)
    fout.write("{}\n".format(out_str))
...
def outputTail(
    ...
    r = hr[1]
    if out_reverse:
        out_str = "head:{}\trel:{}\ttarget_tail:{}".format(
            e_map_reverse[h], r_map_reverse[r], ",".join([e_map_reverse[t] for t in target_tails])
        )
    else:
        out_str = "head:{}\trel:{}\ttarget_tail:{}".format(
            e_map_reverse[h], r_map_reverse[r], ",".join([str(t) for t in target_tails])
        )
    ...
    for t in remap_ts:
        ...
        for gt in remap_gts:
            ...
            if len(target_users) > 0:
                if out_reverse:
                    tmp_str = "{},{},[{}]".format(
                        e_map_reverse[i2kg_map[t]],
                        e_map_reverse[i2kg_map[gt]],
                        ",".join([str(u_map_reverse[u]) for u in target_users]),
                    )
                else:
                ...
                for gt in remap_gts:
                    ...
                    if len(target_users) > 0:
                        if out_reverse:
                            ...
                            tmp_str = "{},{},{},[{}]".format(
                                t, gt, r_map_reverse[tmp_r], ",".join([str(u) for u in target_users])
                            )
                        out_str += "\tusers:{}".format(tmp_str)
    fout.write("{}\n".format(out_str))
...
compareLogs(
    ...
    output_file,
    rel_set=None,
    u_map_reverse=user_vocab_reverse,
    i_map_reverse=item_vocab_reverse,
    e_map_reverse=kg_vocab_reverse,
    r_map_reverse=rel_vocab_reverse,
)
