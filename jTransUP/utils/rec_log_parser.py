...
def parseRecResults(log_filename, model_type):
    ...
    print(
        "parse {} users, avgerage {} correct and {} wrong!".format(
            len(results), correct_num / len(results), wrong_num / len(results)
        )
    )
...
def compareLogs(
    ...
    with open(output_file, "w", encoding="utf-8") as fout:
        for u_id in results1:
            if u_id not in results2 or (users is not None and u_map_reverse[u_id] not in users):
                continue
...
def analysis(
    ...
    if (
        ...
        out_reverse = True
    if out_reverse:
        out_str = "user:{}\ttarget:{}".format(
            u_map_reverse[u_id], ",".join([e_map_reverse[i2kg_map[ti]] for ti in target_items if ti in i2kg_map])
        )
    else:
    ...
    if len(share_prefs) == 0:
        return None
    org_pref = [r_map_reverse[r] for r in share_prefs]
    if "http://dbpedia.org/ontology/starring" not in org_pref or "http://dbpedia.org/ontology/director" not in org_pref:
        return None
    ...
    for tr in zip(remap_tis, target_rel_ids):
        ...
        for gid in remap_gids:
            ...
            if len(gh_ids) == 0 and len(gt_ids) == 0:
                continue
            h_gh = h_ids & gh_ids
            h_gt = h_ids & gt_ids
            t_gh = t_ids & gh_ids
            ...
            if len(h_gh) > 0:
                if out_reverse:
                    tmp_str = "{},{},{},[{}]".format(
                        ...
                        e_map_reverse[gid],
                        r_map_reverse[tr[1]],
                        ",".join([str(e_map_reverse[i]) for i in h_gh]),
                    )
                else:
            ...
            if len(h_gt) > 0:
                if out_reverse:
                    tmp_str = "{},{},{},[{}]".format(
                        ...
                        e_map_reverse[gid],
                        r_map_reverse[tr[1]],
                        ",".join([str(e_map_reverse[i]) for i in h_gt]),
                    )
                else:
            ...
            if len(t_gh) > 0:
                if out_reverse:
                    tmp_str = "{},{},{},[{}]".format(
                        ...
                        e_map_reverse[gid],
                        r_map_reverse[tr[1]],
                        ",".join([str(e_map_reverse[i]) for i in t_gh]),
                    )
                else:
            ...
            if len(t_gt) > 0:
                if out_reverse:
                    tmp_str = "{},{},{},[{}]".format(
                        ...
                        e_map_reverse[gid],
                        r_map_reverse[tr[1]],
                        ",".join([str(e_map_reverse[i]) for i in t_gt]),
                    )
                else:
